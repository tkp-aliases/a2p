...
lint-py:  ## lint python with ruff
	python -m ruff check a2p
	python -m ruff format --check a2p
...
fix-py:  ## autoformat python code with ruff
	python -m ruff check --fix a2p
	python -m ruff format a2p
...
test:  ## run python tests
	python -m pytest -v a2p/tests

coverage:  ## run tests and collect test coverage
	python -m pytest -v a2p/tests --cov=a2p --cov-report term-missing --cov-report xml
